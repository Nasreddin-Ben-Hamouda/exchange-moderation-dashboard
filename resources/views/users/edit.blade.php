...
<div class="content">
  ...
  <div class="row align-items-center">
    <div class="col-lg-5 col-md-6 col-sm-8 ml-auto mr-auto" style="margin-top:20px;">
      ...
      @endif
      <form class="form" method="POST" action="{{ URL('user/'.$moderator->id) }}">
        @csrf
        @method("PUT")
        <div class="card card-login card-hidden mb-3">
          <div class="card-header card-header-info text-center">
            <h4 class="card-title"><strong>{{ __('Mettre à jour le modérateur') }}</strong></h4>
          </div>
          <div class="card-body ">
            ...
            <div class="bmd-form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
              <div class="input-group">
                ...
                <input value="{{ $moderator->name }}" type="text" name="name" class="form-control" placeholder="{{ __('Nom Prénom...') }}" value="{{ old('name') }}" required>
              </div>
              @if ($errors->has('name'))
            ...
            <div class="bmd-form-group{{ $errors->has('email') ? ' has-danger' : '' }} mt-3">
              <div class="input-group">
                ...
                <input  value="{{ $moderator->email }}" type="email" name="email" class="form-control" placeholder="{{ __('Email...') }}" value="{{ old('email') }}" required>
              </div>
              @if ($errors->has('email'))
          ...
          <div class="card-footer justify-content-center">
             <div class="pl-3">
                <button type="submit" class="btn btn-success  btn-lg">{{ __('Modifier le modérateur') }}</button>
             </div>
          </div>
        </div>
      </form>
